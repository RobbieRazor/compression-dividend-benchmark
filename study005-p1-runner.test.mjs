...
const runnerMetadata = readJson(
  './data/CD-WORKLOAD-20260830-005-p1-runner-metadata.json'
);
const provider = readJson('./data/CD-WORKLOAD-20260830-005-provider.json');
...
test('Study 005 P1 runner metadata pins the exact runner and preregistration', () => {
  assert.equal(
  ...
  assert.equal(
    ...
    sha256('./scripts/run-study005-p1.mjs')
  );
  assert.equal(
    runnerMetadata.preregistration_manifest.sha256,
    sha256('./data/CD-WORKLOAD-20260830-005-preregistration-manifest.json')
  );
  assert.equal(
...
test('runner preflight reproduces the exact request without API activity', () => {
  const result = spawnSync(
    'node',
    ['scripts/run-study005-p1.mjs', '--preflight'],
    {
  ...
  assert.match(stdout, /OBSERVATION_RESERVED: CD-005-P1-0001/);
  assert.match(stdout, /MODEL_VISIBLE_INPUT_COUNT: 4/);
  assert.match(stdout, /MODEL_INPUT_BYTES: 171471/);
  assert.match(
    stdout,
  ...
  assert.match(stdout, /STUDY004_FAILURE_DIAGNOSTICS_VISIBLE: false/);
...
test('runner reads only the four frozen model-visible inputs', () => {
  const runner = readFileSync(
    new URL('./scripts/run-study005-p1.mjs', import.meta.url),
    'utf8'
  );

  assert.equal(taskMetadata.model_visible_inputs.length, 4);
  assert.doesNotMatch(
    runner,
  ...
  assert.doesNotMatch(
    runner,
    /CD-WORKLOAD-20260830-005-enriched-biography-public-authority-package\.json/
  );
  assert.doesNotMatch(
    runner,
    /CD-WORKLOAD-20260830-005-quality-contract\.json/
  );
  assert.doesNotMatch(runner, /evaluate-study005\.py/);
  assert.doesNotMatch(runner, /CD-004-P1-0001-output\.json/);
  assert.doesNotMatch(runner, /CD-004-P1-0001-quality-evaluation\.json/);
  assert.doesNotMatch(runner, /structural-adjudication\.json/);
  ...
  assert.equal(runnerMetadata.visibility_boundary.study004_model_output_visible, false);
  assert.equal(runnerMetadata.visibility_boundary.study004_quality_evaluation_visible, false);
  assert.equal(runnerMetadata.visibility_boundary.study004_structural_adjudication_visible, false);
...
test('only reserved observation 0001 is accepted by the runner', () => {
  const result = spawnSync(
    'node',
    ['scripts/run-study005-p1.mjs', '0002'],
    {
      cwd: new URL('.', import.meta.url),
      encoding: 'utf8',
  ...
  assert.match(
    ...
    /Usage: node scripts\/run-study005-p1\.mjs --preflight OR 0001/
  );
});


test('reserved measurement artifacts do not exist at runner freeze', () => {
  for (const suffix of [
    'response.json',
    'output.json',
    'output.txt',
    ...
    assert.equal(
      existsSync(
        new URL(`./data/raw/CD-005-P1-0001-${suffix}`, import.meta.url)
      ),
      false,
      suffix
    );
