...
STUDY_ID = "CD-WORKLOAD-20260830-006"
MODULE_ID = "CD006-CANONICAL-LEXICAL-ENRICHED-BIOGRAPHY-001"
OBSERVATION_ID = "CD-006-P1-0001"
ADJUDICATOR_ID = "CD006-P1-CANONICAL-FIDELITY-ADJ-1.0"

ROOT = Path.cwd()

TARGET_FILE = ROOT / "data/CD-WORKLOAD-20260830-005-enriched-biography-target-representation.json"
QUALITY_FILE = ROOT / "data/CD-WORKLOAD-20260830-006-quality-contract.json"
OUTPUT_FILE = ROOT / "data/raw/CD-006-P1-0001-output.json"
MEASUREMENT_FILE = ROOT / "data/raw/CD-006-P1-0001-measurement.json"
EVALUATION_FILE = ROOT / "data/raw/CD-006-P1-0001-quality-evaluation.json"
STUDY005_EVALUATION_FILE = ROOT / "data/raw/CD-005-P1-0001-quality-evaluation.json"
ADJUDICATOR_FILE = ROOT / "scripts/adjudicate-study006-p1.py"
METADATA_FILE = ROOT / "data/CD-WORKLOAD-20260830-006-p1-canonical-fidelity-adjudicator-metadata.json"
RESULT_FILE = ROOT / "data/raw/CD-006-P1-0001-canonical-fidelity-adjudication.json"


def fail(message):
    print("STUDY006_ADJUDICATOR_ERROR:", message)
    raise SystemExit(1)
...
def rights_adjudication(expected, observed):
    ...
    return {
        "frozen_exact_gate_pass": exact_fact_count == 13,
...
def build_adjudication(target_record, output, measurement, evaluation, study005_evaluation):
    ...
    if category_counts != {
        "ONTOLOGY_TYPE_SUBSTITUTION": 6,
        "CANONICAL_DESCRIPTOR_PARAPHRASE": 4,
        "TRADEMARK_MARK_OMISSION": 4
    }:
    ...
    return {
        ...
        "purpose": "Quantify the frozen Study 006 lexical intervention against Study 005 without changing quality, authorizing retry, opening P2/P3, or performing an economic comparison.",
        "frozen_evidence": {
        ...
        "cross_study_comparison": {
            "study005_subject_fact_pass_count": study005_evaluation["result"]["subject"]["fact_pass_count"],
            "study006_subject_fact_pass_count": subject_pass,
            "subject_exact_fact_gain": subject_pass - study005_evaluation["result"]["subject"]["fact_pass_count"],
            "study005_relationship_target_fact_pass_count": sum(item["target_fact_pass_count"] for item in study005_evaluation["result"]["relationships"]["results"]),
            "study006_relationship_target_fact_pass_count": relationship_pass,
            "relationship_exact_fact_gain": relationship_pass - sum(item["target_fact_pass_count"] for item in study005_evaluation["result"]["relationships"]["results"]),
            "study005_rights_pass": study005_evaluation["result"]["retrieval_rights"]["pass"],
            "study006_rights_pass": evaluation["result"]["retrieval_rights"]["pass"],
        ...
        "primary_diagnostic_label": "SUBJECT_AND_RIGHTS_FIDELITY_GAIN_WITH_UNCHANGED_RELATIONSHIP_EXACTNESS",
        "secondary_diagnostic_label": "CANONICAL_RELATIONSHIP_VALUE_FIDELITY_REMAINS_INSUFFICIENT",
...
study005_evaluation = read_json(STUDY005_EVALUATION_FILE)
...
if len(sys.argv) == 2 and sys.argv[1] == "--preflight":
    print("========================================")
    print("STUDY 006 P1 CANONICAL FIDELITY ADJUDICATION PREFLIGHT")
    print("========================================")
    ...
    print("SUBJECT_EXACT_FACT_GAIN_VS_STUDY005:", adjudication["cross_study_comparison"]["subject_exact_fact_gain"])
    print("RELATIONSHIP_EXACT_FACT_GAIN_VS_STUDY005:", adjudication["cross_study_comparison"]["relationship_exact_fact_gain"])
...
if len(sys.argv) != 2 or sys.argv[1] != "P1-0001":
    fail("Usage: python3 scripts/adjudicate-study006-p1.py --preflight OR P1-0001")
...
print("========================================")
print("STUDY 006 P1 CANONICAL FIDELITY ADJUDICATION")
print("========================================")
print("OBSERVATION_ID:", OBSERVATION_ID)
print("ADJUDICATOR_ID:", ADJUDICATOR_ID)
print("FROZEN_QUALITY_GATE_PASS:", False)
