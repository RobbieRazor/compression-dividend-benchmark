...
CONTRACT_ID = "CD006-QC-1.0"
TARGET_ID = "CD005-ENRICHED-BIOGRAPHY-SCHEMA-TARGET-1.0"
EVALUATOR_ID = "CD006-STRICT-CANONICAL-EVAL-1.0"

ROOT = Path.cwd()

QC_FILE = ROOT / "data/CD-WORKLOAD-20260830-006-quality-contract.json"
...
TASK_METADATA_FILE = ROOT / "data/CD-WORKLOAD-20260830-006-task-metadata.json"
PROVIDER_FILE = ROOT / "data/CD-WORKLOAD-20260830-006-provider.json"
EVALUATOR_FILE = ROOT / "scripts/evaluate-study006.py"


def fail(message):
    print("STUDY006_EVALUATOR_ERROR:", message)
    raise SystemExit(1)
...
if authority_control.get("status") != "EXACT_STUDY005_AUTHORITY_SCHEMA_AND_TARGET_REUSE_FROZEN":
    fail("Study 006 authority and target reuse is not frozen.")
...
if len(sys.argv) == 2 and sys.argv[1] == "--preflight":
    ...
    print("========================================")
    print("STUDY 006 STRICT CANONICAL EVALUATOR PREFLIGHT")
    print("========================================")
...
if len(sys.argv) != 2 or not re.fullmatch(r"P[123]-\d{4}", sys.argv[1]):
    fail("Usage: python3 scripts/evaluate-study006.py --preflight OR P1-0001 OR P2-0001 OR P3-0001")

path_number = sys.argv[1]
observation_id = "CD-006-" + path_number
...
print("========================================")
print("STUDY 006 STRICT CANONICAL QUALITY EVALUATION")
print("========================================")
