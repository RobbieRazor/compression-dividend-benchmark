...
def fail(message):
    print("STUDY003_EVALUATOR_ERROR:", message)
    raise SystemExit(1)
...
if qc["transport_normalization"]["field"] != "@id":
    fail(
        "Unexpected transport-normalized field."
    )

if qc["representation_selection"]["alternate_public_nodes_accepted"] is not False:
    fail(
        "Alternate public nodes must not be accepted."
    )
...
if exact_fields != expected_exact_fields:
    fail(
        "Frozen exact-field order or contents differ from evaluator expectation."
    )
...
if expected_public_id not in accepted_ids:
    fail(
        "Canonical public Biography Plate ID missing from accepted identities."
    )

if len(sys.argv) == 2 and sys.argv[1] == "--preflight":
    print("========================================")
    print("STUDY 003 QUALITY EVALUATOR PREFLIGHT")
    print("========================================")
    ...
    print("ALTERNATE_PUBLIC_NODES_ACCEPTED: false")
...
if (
    ...
    fail(
        "Usage: python3 scripts/evaluate-study003.py --preflight OR 0001"
    )
...
if not output_file.exists():
    fail(
        "Observation output file not found."
    )

if not measurement_file.exists():
    fail(
        "Observation measurement file not found."
    )

if evaluation_file.exists():
    fail(
        "Evaluation already exists. Refusing overwrite."
    )
...
if (
    ...
    fail(
        "Raw measurement is not in NOT_YET_EVALUATED state."
    )

if not isinstance(output, dict):
    fail(
        "Observation output is not a JSON object."
    )
...
evaluation = {
    ...
    "evaluation_policy": {
        "deep_exact":
            "Python deep equality against the frozen explicit public state surface. Dict key order is insignificant; list order is significant.",

        "transport":
            "Observed @id must belong to the predeclared accepted transport-identity set.",
...
print("========================================")
print("STUDY 003 QUALITY EVALUATION")
print("========================================")
