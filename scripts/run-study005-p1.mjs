...
const STUDY_ID = 'CD-WORKLOAD-20260830-005';
const MODULE_ID = 'CD005-SCHEMA-CONSTRAINED-ENRICHED-BIOGRAPHY-001';
const PATH_ID = 'P1';
const CONTRACT_ID = 'CD005-QC-1.0';
const ROOT = process.cwd();

const taskFile = path.join(
  ROOT,
  'prompts/CD-WORKLOAD-20260830-005-neutral-task.txt'
);
...
function fail(message) {
  console.error('STUDY005_P1_ERROR:', message);
  process.exit(1);
}
...
for (const field of [
  'design_frozen',
  'neutral_schema_frozen',
  'public_authority_evidence_frozen',
  'target_representation_frozen',
...
for (const field of [
  ...
  if (preregistration.frozen_state[field] !== false) {
    fail('Unexpected premeasurement state: ' + field);
  }
}
...
if (
  preregistration.payment_gate.p2_probe_allowed_now !== false ||
  preregistration.payment_gate.p3_payment_allowed_now !== false
) {
...
if (
  ...
  fail('P1 feasibility observation or retry boundary changed.');
}

for (const field of [
  'study005_authority_freeze_control_visible_to_p1',
  ...
  'study004_model_output_visible_to_p1',
  'study004_quality_evaluation_visible_to_p1',
  'study004_structural_adjudication_visible_to_p1',
...
const visibleFiles = {
  ...
  value_free_neutral_schema: schemaFile,
...
if (mode === '--preflight') {
  console.log('========================================');
  console.log('STUDY 005 P1 SCHEMA FEASIBILITY RUNNER PREFLIGHT');
  console.log('========================================');
  console.log('STUDY_ID:', STUDY_ID);
  console.log('MODULE_ID:', MODULE_ID);
  console.log('PATH:', PATH_ID);
  console.log('OBSERVATION_RESERVED:', 'CD-005-P1-0001');
  ...
  console.log('STUDY004_FAILURE_DIAGNOSTICS_VISIBLE:', false);
...
if (mode !== '0001') {
  fail('Usage: node scripts/run-study005-p1.mjs --preflight OR 0001');
}
...
const observationId = 'CD-005-P1-0001';
...
const measurement = {
  ...
  request: {
    ...
    study004_failure_diagnostics_visible: false,
